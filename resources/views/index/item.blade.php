				...
				<div class="fl preview-wrap">
					...
					<div class="zoom">
						...
						<div id="preview" class="spec-preview">
							<span class="jqzoom">
								<img jqimg="{{$data['sku_img'][0]['bg_path']}}" src="{{$data['sku_img'][0]['md_path']}}" />
							</span>
						</div>
						<!--下方的缩略图-->
						<div class="spec-scroll">
							...
							<div class="items">
								<ul>
									@foreach ($data['sku_img'] as $v)
										<li><img src="{{$v['md_path']}}" bimg="{{$v['bg_path']}}" onmousemove="preview(this)" /></li>
									@endforeach
								</ul>
							</div>
							<a class="next">&gt;</a>
						...
						<div id="specification" class="summary-wrap clearfix">
							@foreach ($data['attr_name'] as $v)
								<dl>
									...
									@foreach ($v['get_val'] as $v2)
										<dd>
											<a href="javascript:;" onclick="attr(this,{{$v['id']}},{{$v2['id']}})"
											@if (strstr($data['attr'],$v['id'].':'.$v2['id']))
												class="selected"
											@endif 
											attr="{{$v['id'].':'.$v2['id']}}">{{$v2['attr_value']}}</a>
										</dd>
									@endforeach
								</dl>
							@endforeach
...
<script>
	function attr(obj,name_id,value_id)
	{
		...
		for(var i=0;i<num;i++)
		{
			if(attr_str=='')
			{
				attr_str = $('.selected')[i].attributes['attr'].nodeValue
			}
			else
			{
				attr_str+= ','+$('.selected')[i].attributes['attr'].nodeValue
		...
		$.ajax({
			type:'GET',
			url:"{{Route('GoodsGetInfo')}}",
			data:{attr_str:attr_str,goods_id:"{{$data['goods_id']}}"},
			dataType:"JSON",
