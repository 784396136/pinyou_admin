...
<body>
    <div class="margin clearfix">
        <div class="Shops_Audit">
            @if ($data['count']!=0)
                <div class="prompt">当前共有<b>{{$data['count']}}</b>家店铺申请入驻</div>
            @else
                <div class="prompt">当前还未有商家申请入驻</div>
            @endif
            <!--申请列表-->
                    ...
                    <tbody>
                        @foreach ($data['data'] as $v)
                                <tr>
                                    ...
                                    <td>{{$v->created_at}}</td>
                                    <td class="td-manage">
                                        <a title="店铺详细" href="{{Route('ShopDetailed',['id'=>$v->id])}}" class="btn btn-xs btn-info Refund_detailed">详细</a>
